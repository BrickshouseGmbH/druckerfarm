...
type ServerConn struct {
	...
	tlsConn     *tls.Conn       // control TLS conn for session reuse (some implicit-FTPS servers)
	host        string
...
func Dial(addr string, options ...DialOption) (*ServerConn, error) {
	...
	// Store TLS conn for session reuse on data connections (required by some FTPS servers)
	var tlsControlConn *tls.Conn
	if tlsC, ok := tconn.(*tls.Conn); ok {
		tlsControlConn = tlsC
	}
	c := &ServerConn{
		options:  do,
...
func (c *ServerConn) openDataConn() (net.Conn, error) {
	...
	if c.options.tlsConfig != nil {
		...
		// Some implicit-FTPS servers require TLS session reuse on data connections.
		// The ClientSessionCache must be shared and ServerName must match.
		dataConfig := c.options.tlsConfig.Clone()
		dataConfig.ServerName = c.host
		// Ensure control conn has completed handshake so session is cached
		if c.tlsConn != nil {
			// Touch connection state to ensure session ticket is in cache
			_ = c.tlsConn.ConnectionState().Version
		}
		tlsDataConn := tls.Client(conn, dataConfig)
		if err := tlsDataConn.Handshake(); err != nil {
			conn.Close()
			return nil, err
		}
		return tlsDataConn, nil
	}
